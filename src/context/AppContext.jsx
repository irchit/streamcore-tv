...
export const AppProvider = ({ children }) => {
  ...
  const [subscriptions, setSubscriptions] = useState([
    { id: "digi-ro", name: "Digi Romania (Alle Kanäle)", status: "active", price: "2.49", desc: "Kompletter Zugriff pe toate canalele Digi: Sport, News, Dokumentationen und Film-Pakete." },
    { id: "telekom-de", name: "Telekom Deutschland TV", status: "inactive", price: "5.99", desc: "30 deutsche Premium-TV-Kanäle live inklusive vollständiger Auto-Recording-Unterstützung." },
    { id: "amazon-prime", name: "Amazon Prime Video", status: "inactive", price: "3.99", desc: "Die neuesten 15 Blockbuster-Filme und 15 exklusiven TV-Shows direkt in der Plattform." },
    { id: "netflix", name: "Netflix Premium Lite", status: "inactive", price: "6.49", desc: "Eine kuratierte Auswahl von 30 weltweiten Top-Titeln und exklusiven Netflix Originals." },
    { id: "disney-plus", name: "Disney+ Sci-Fi & Hero", status: "inactive", price: "4.99", desc: "Das gesamte Star Wars Universum und alle Marvel MCU Highlights bis einschließlich Phase 4." }
  ]);
  ...
  const activateSubscription = (subId) => {
    setSubscriptions(prevSubs => 
      prevSubs.map(sub => sub.id === subId ? { ...sub, status: "active" } : sub)
    );
  };

  const deactivateSubscription = (subId) => {
    setSubscriptions(prevSubs => 
      prevSubs.map(sub => sub.id === subId ? { ...sub, status: "expiring" } : sub)
    );
  };

  const terminateSubscription = (subId) => {
    setSubscriptions(prevSubs => 
      prevSubs.map(sub => sub.id === subId ? { ...sub, status: "inactive" } : sub)
    );
  };
